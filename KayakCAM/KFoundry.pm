...
sub kayak_to_scad($$) {
    ...
    print $fh join("\n",
		...
		(map {
		    my $x = $_ * 50;
		    "polyline3d(". to_scad([ map { [$x ,$_->[0],     $_->[1]] } @{getThetaCurve(make_cs_at_x($yak, $x),50)}]) . ", 6);\n" .
		    "polyline3d(". to_scad([ map { [$x ,$_->[0] * -1,$_->[1]] } @{getThetaCurve(make_cs_at_x($yak, $x),50)}]) . ", 6);"
...
sub kayak_to_mesh($) {
	...
	my $bez1 = make_cs_at_x($yak,$x1);
	my $bez2 = make_cs_at_x($yak,$x2);
